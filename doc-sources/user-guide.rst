...
It can also be ``single-peaked`` for criteria where intermediate values correspond to upper categories.
These single-peaked criteria are described in :doc:`their own document <single-peaked>`.
...
But this lacks generality and does not apply for for single-peaked criteria.
...
For criteria with ``increasing`` or ``decreasing`` preference direction, the method is always ``kind: thresholds``,
...
Note that this list is not a profile: it does not describe the limits between categories, but the consecutive limits related to a given criterion.
The matrix made of these lists is the transposed of the matrix made of the profiles.

For criteria with ``single-peaked`` preference direction, the method is ``kind: intervals``.
It's described in details in our :doc:`single-peaked criteria documentation <single-peaked>`.
...
``single-peaked``                   ``intervals``             ``intervals``
==================================  ========================  ==========================
...
The ``--forbid-real-criteria`` option forbids ``real`` criteria.
...
Another set of options selects the ``preference_direction`` of the criteria in the generated problem.
By default, all criteria have ``increasing`` preference direction.
For numerical criteria, ``--allow-decreasing-criteria`` lets you generate problems with ``decreasing`` criteria,
and ``--allow-single-peaked-criteria`` lets you generate problems with ``single-peaked`` criteria.
The ``--forbid-increasing-criteria`` option forbids ``increasing`` criteria.
The ``preference_direction`` of each criterion is chosen pseudo-randomly among the allowed ones.
...
.. code:: yaml
    ...
    # Reproduction command (with lincs version 2.0.0a7.dev0): lincs learn classification-model problem.yml learning-set.csv --model-type ucncs --ucncs.strategy sat-by-coalitions
    kind: ncs-classification-model
    format_version: 1
    accepted_values:
      - kind: thresholds
